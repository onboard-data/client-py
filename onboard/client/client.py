...
class APIClient(ClientBase):
    ...
    @deprecation.deprecated(
        deprecated_in="1.3.0", details="Use stream_point_timeseries instead"
    )
    @json
    ...
    def stream_point_timeseries(
        self, query: TimeseriesQuery, resample_mins: Optional[int] = None
    ) -> Iterator[PointData]:
...
class DevelopmentAPIClient(APIClient):
    def __init__(
        ...
        super().__init__(
            "https://devapi.onboarddata.io", user, pw, api_key, token, retry=retry
        )
...
class ProductionAPIClient(APIClient):
    def __init__(
        ...
        super().__init__(
            "https://api.onboarddata.io", user, pw, api_key, token, retry=retry
        )
